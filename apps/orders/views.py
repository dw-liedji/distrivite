...
class OrgCommercialListView(
    ...
    template_name = "orders/commercial_list.html"
    context_object_name = "commercials"
    paginate_by = 30
    permission_required = ("orders.view_commercial",)

    def get_template_names(self):
        if self.request.htmx:
            if self.request.headers.get("HX-Request-Source") == "sidebar":
                return ["orders/commercial_list.html#list"]
            return ["orders/commercial_list.html#list"]
        return ["orders/commercial_list.html"]

    def get_queryset(self):
        return models.Commercial.objects.filter(
...
class OrgCommercialAddView(
    ...
    model = models.Commercial
    form_class = forms.CommercialForm
    template_name = "orders/commercial_add.html"
    permission_required = ("orders.add_commercial",)

    def get_template_names(self):
        if self.request.htmx:
            return ["orders/commercial_add.html#add"]
        return ["orders/commercial_add.html"]

    def form_valid(self, form):
        consultation = form.save()
        messages.success(self.request, f"Commercial {consultation} saved successfully")
        return super().form_valid(form)

    def form_invalid(self, form):
        messages.success(
            self.request, f"Commercial failed to be saved. Somthing went wrong."
        )
    ...
    def get_success_url(self):
        return reverse_lazy(
            "organization_features:orders:commercial_list",
            kwargs={"organization": self.request.organization.slug},
        )


class OrgCommercialChangeView(
    LoginRequiredMixin,
    mixins.OrgPermissionRequiredMixin,
    mixins.MembershipRequiredMixin,
    mixins.OrgFormMixin,
    ...
    model = models.Commercial
    form_class = forms.CommercialForm
    template_name = "orders/commercial_change.html"
    permission_required = ("orders.change_commercial",)

    def get_template_names(self):
        if self.request.htmx:
            return ["orders/commercial_change.html#change"]
        return ["orders/commercial_change.html"]

    def form_valid(self, form):
        consultation = form.save()
        messages.success(
            self.request, f"Commercial {consultation} updated successfully"
        )
        return super().form_valid(form)
...
class OrgCommercialDeleteView(
    ...
    model = models.Commercial
    template_name = "orders/commercial_confirm_delete.html"
    permission_required = ("orders.delete_commercial",)
...
class OrgCommercialDetailView(
    ...
    def get_template_names(self):
        if self.request.htmx:
            return ["orders/commercial_detail.html#detail"]
        return ["orders/commercial_detail.html"]
...
class OrgPrepaidAccountListView(
    LoginRequiredMixin,
    mixins.OrgPermissionRequiredMixin,
    mixins.MembershipRequiredMixin,
    FilterView,
    ...
    template_name = "orders/prepaid_account_list.html"
    context_object_name = "prepaid_accounts"
    paginate_by = 30
    permission_required = ("orders.view_prepaid_account",)
    filterset_class = orders_filters.PrepaidAccountFilter

    def get_template_names(self):
        if self.request.htmx:
            if self.request.headers.get("HX-Request-Source") == "sidebar":
                return ["orders/prepaid_account_list.html#list"]
            return ["orders/prepaid_account_list.html#list"]
        return ["orders/prepaid_account_list.html"]

    def get_queryset(self):
        return models.PrepaidAccount.objects.filter(
            organization__in=self.request.organization.get_descendants(
                include_self=True
            )
        ).order_by("-created")
...
class OrgPrepaidAccountAddView(
    LoginRequiredMixin,
    mixins.OrgPermissionRequiredMixin,
    mixins.MembershipRequiredMixin,
    mixins.OrgFormMixin,
    ...
    model = models.PrepaidAccount
    form_class = forms.PrepaidAccountForm
    template_name = "orders/prepaid_account_add.html"
    permission_required = ("orders.add_prepaid_account",)

    def get_template_names(self):
        if self.request.htmx:
            return ["orders/prepaid_account_add.html#add"]
        return ["orders/prepaid_account_add.html"]

    def form_valid(self, form):
        consultation = form.save()
        messages.success(
            self.request, f"PrepaidAccount {consultation} saved successfully"
        )
        return super().form_valid(form)

    def form_invalid(self, form):
        messages.success(
            self.request, f"PrepaidAccount failed to be saved. Somthing went wrong."
        )
        return super().form_invalid(form)
    ...
    def get_success_url(self):
        return reverse_lazy(
            "organization_features:orders:prepaid_account_list",
            kwargs={"organization": self.request.organization.slug},
        )


class OrgPrepaidAccountChangeView(
    LoginRequiredMixin,
    mixins.OrgPermissionRequiredMixin,
    mixins.MembershipRequiredMixin,
    mixins.OrgFormMixin,
    ...
    model = models.PrepaidAccount
    form_class = forms.PrepaidAccountForm
    template_name = "orders/prepaid_account_change.html"
    permission_required = ("orders.change_prepaid_account",)

    def get_template_names(self):
        if self.request.htmx:
            return ["orders/prepaid_account_change.html#change"]
        return ["orders/prepaid_account_change.html"]

    def form_valid(self, form):
        consultation = form.save()
        messages.success(
            self.request, f"PrepaidAccount {consultation} updated successfully"
        )
        return super().form_valid(form)
...
class OrgPrepaidAccountDeleteView(
    LoginRequiredMixin,
    mixins.OrgPermissionRequiredMixin,
    mixins.MembershipRequiredMixin,
    DeleteView,
    ...
    model = models.PrepaidAccount
    template_name = "orders/prepaid_account_confirm_delete.html"
    permission_required = ("orders.delete_prepaid_account",)
...
class OrgPrepaidAccountDetailView(
    LoginRequiredMixin,
    mixins.OrgPermissionRequiredMixin,
    mixins.MembershipRequiredMixin,
    DetailView,
    ...
    model = models.PrepaidAccount
    context_object_name = "prepaid_account"
    template_name = "orders/prepaid_account_detail.html"
    permission_required = ("orders.view_prepaid_account",)

    def get_template_names(self):
        if self.request.htmx:
            return ["orders/prepaid_account_detail.html#detail"]
        return ["orders/prepaid_account_detail.html"]
...
class OrgSupplierPaymentListView(
    LoginRequiredMixin,
    mixins.OrgPermissionRequiredMixin,
    mixins.MembershipRequiredMixin,
    # AdminRequiredMixin,
    ...
    permission_required = ("orders.view_supplierpayment",)
    filterset_class = orders_filters.SupplierPaymentFilter

    def get_template_names(self):
        if self.request.htmx:
            if self.request.headers.get("HX-Request-Source") == "sidebar":
                return ["orders/supplier_payment_list.html#list"]
            return ["orders/supplier_payment_list.html#list"]
        return ["orders/supplier_payment_list.html"]

    def get_queryset(self):

        return models.SupplierPayment.objects.for_organization(
            organization=self.request.organization
        )
...
class OrgSupplierPaymentCreateView(
    LoginRequiredMixin,
    mixins.OrgPermissionRequiredMixin,
    mixins.MembershipRequiredMixin,
    # AdminRequiredMixin,
    ...
    template_name = "orders/supplier_payment_add.html"
    success_message = "supplier_payments %(name)s successfully created!"

    permission_required = ("orders.add_supplierpayment",)

    def get_template_names(self):
        if self.request.htmx:
            return ["orders/supplier_payment_add.html#add"]
        return ["orders/supplier_payment_add.html"]

    @core_decorators.preserve_query_params()
    def get_success_url(self):
        if "save" in self.request.POST:
            return reverse_lazy(
                "organization_features:orders:supplier_payment_list",
                kwargs={
                    "organization": self.request.organization.slug,
                },
            )
            ...
            return reverse_lazy(
                "organization_features:orders:supplier_payment_add",
                kwargs={
                    "organization": self.request.organization.slug,
                },
            )
    ...
    def get_form_kwargs(self):
        kwargs = super(OrgSupplierPaymentCreateView, self).get_form_kwargs()
    ...
    def form_valid(self, form):
        with transaction.atomic():
            ...
            supplier_payment.payer = self.request.user
            supplier_payment.save()

            if supplier_payment.amount > 0.0:
                cashflow_models.Deposit.objects.create(
                    organization=self.request.organization,
                    organization_user=self.request.organization_user,
                    amount=supplier_payment.amount,
                    accounting_date=supplier_payment.accounting_date,
                    cash_register=supplier_payment.cash_register,
                    reason=f"Paiement de {supplier_payment.supplier } par {supplier_payment.payer}",
                )
...
class OrgSupplierPaymentUpdateView(
    LoginRequiredMixin,
    mixins.OrgPermissionRequiredMixin,
    mixins.MembershipRequiredMixin,
    # AdminRequiredMixin,
    ...
    model = models.SupplierPayment
    form_class = forms.SupplierPaymentForm
    template_name = "orders/supplier_payment_edit.html"
    permission_required = ("orders.change_supplierpayment",)

    def get_template_names(self):
        if self.request.htmx:
            return ["orders/supplier_payment_change.html#change"]
        return ["orders/supplier_payment_change.html"]
    ...
    def get_form_kwargs(self):
        kwargs = super(OrgSupplierPaymentUpdateView, self).get_form_kwargs()
        kwargs.update(
            {
                "organization": self.request.organization,
                # "organization_user": self.request.organization_user,
...
class OrgSupplierPaymentDeleteView(
    LoginRequiredMixin,
    mixins.OrgPermissionRequiredMixin,
    mixins.MembershipRequiredMixin,
    # AdminRequiredMixin,
    ...
    model = models.SupplierPayment
    template_name = "orders/supplier_payment_confirm_delete.html"
    permission_required = ("orders.delete_supplierpayment",)
...
class OrgSupplierPaymentDetailView(
    LoginRequiredMixin,
    mixins.OrgPermissionRequiredMixin,
    mixins.MembershipRequiredMixin,
    # AdminRequiredMixin,
    ...
    model = models.SupplierPayment
    context_object_name = "supplier_payment"
    template_name = "orders/supplier_payment_detail.html"
    permission_required = ("orders.view_supplierpayment",)

    def get_template_names(self):
        if self.request.htmx:
            return ["orders/supplier_payment_detail.html#detail"]
        return ["orders/supplier_payment_detail.html"]

    def get_object(self, queryset=None):
        self.object = get_object_or_404(
            models.SupplierPayment, pk=self.kwargs.get("pk")
        )
        return self.object
