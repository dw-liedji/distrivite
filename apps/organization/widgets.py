...
class TabularPermissionsWidget(FilteredSelectMultiple):
    class Media:
        js = ("organization/permissions.js",)
    ...
    def __init__(
        ...
        self.managed_perms = []
        self.input_name = input_name  # in case of UserAdmin, it's 'permissions', GroupAdmin it's 'permissions'
        self.hide_original = True

    def render(self, name, value, attrs=None, renderer=None):
        choices = self.choices
        apps_available = OrderedDict()  # []  # main container to send to template
        permissions = Permission.objects.filter(id__in=value or []).values_list(
            "id", flat=True
        )
        all_perms = (
            ...
            for model_name in app.models:
                ...
                for content_type in content_types:
                    if (
                        ...
                        and content_type.app_label == app.label
                    ):
                        ...
                        view_perm_id = (
                            codename_id_map.get(
                                "%s_%s" % (view_perm_name, content_type.pk), False
                            )
                        ...
                        add_perm_id = (
                            codename_id_map.get(
                                "%s_%s" % (add_perm_name, content_type.pk), False
                            )
                        ...
                        change_perm_id = (
                            codename_id_map.get(
                                "%s_%s" % (change_perm_name, content_type.pk), False
                            )
                        ...
                        delete_perm_id = (
                            codename_id_map.get(
                                "%s_%s" % (delete_perm_name, content_type.pk), False
                            )
                        ...
                        if model._meta.permissions:
                            ...
                            for codename, perm_name in model._meta.permissions:
                                c_perm_id = codename_id_map.get(
                                    "%s_%s" % (codename, content_type.pk), False
                                )
                                verbose_name = TRANSLATION_FUNC(
                                    codename, perm_name, content_type.pk
                                )
